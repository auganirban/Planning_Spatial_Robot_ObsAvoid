function[] = randomized_visualization2(node_index, box_vertices_array, box_normals_array, plot_fig)
    global tree; global g_intermediate;
% % %     % If you want the video, please uncomment the lines with %%%. 
% % %     myVideo = VideoWriter('myVideoFile'); %open video file
% % %     myVideo.FrameRate = 30;  %can adjust this, 5 - 10 works well for me
% % %     open(myVideo);
    for indx = 1:length(node_index)-1
        [g_current, ~, ~, ~, g_intermediate] = FK_URDF(tree.nodesAng(node_index(indx+1), :));
        get_collision_info4(g_intermediate, box_vertices_array, box_normals_array);
% % %         plot_motion(plot_fig, myVideo);
        plot_motion(plot_fig);
    end
end

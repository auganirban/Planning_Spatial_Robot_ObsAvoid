function [contact_nrml_array, dist_array, jcon_array] = get_collision_info4_graspobj(g_intermediate, box_vertices_array, box_normals_array)
    ...
    dist_array = 1000*ones(1, 7+grasped_obj_con); contact_nrml_array = zeros(6, 7+grasped_obj_con); contact_pts_array = zeros(3, 2, 7+grasped_obj_con);
    jcon_array = zeros(6, 7, 7+grasped_obj_con);
    for box_num = 1:4  % loop through the number of obstacles
        ...
        for num_link = 1:length(dist_array)
            if num_link <= 7
...
%             end
    ...
    % Ignore the collision information of first link and include grasped
    % object collision information.
    if ~isempty(grasped_box)
        contact_nrml_array = contact_nrml_array(:, 2:end);
        dist_array = dist_array(1, 2:end);
        jcon_array = jcon_array(:, :, 2:end);
    end

...
theta1 = [-0.2866, -0.8894, -0.3209, 2.0856, 0.6907, -1.1725, 0.5191]';
...
beta = 0.02; Tau = 0.5; h = 0.010; reach_dist = 0.005;
...
%             0,      0,      0, 1.0000];
temp_rotm = [-0.0123,0.1381,0.9903;-0.0578,0.9887,-0.1386;-0.9983,-0.0589,-0.0042];
g2 = [temp_rotm, [window_position(1)+0.050; window_position(2); window_position(3)]; zeros(1, 3), 1];
...
%     [m, position_err] = distDQ(ResDQ, currentDQ);
    [rotation_err, position_err] = distDQ(ResDQ, finalDQ);

    if position_err < reach_dist && rotation_err < 0.01
